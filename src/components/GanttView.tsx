...
import { ChevronRight, ChevronDown, GitBranch, GripVertical, Archive } from 'lucide-react'
import {
...
function calcCriticalSet(tasks: Record<string, Task>): Set<string> {
  ...
  for (const task of Object.values(tasks)) {
    ...
    for (const predId of deps) {
      ...
      if (slack <= 1) { critical.add(predId); critical.add(task.id) }
    }
  }
  return critical
}

// ── SortableRow wrapper (applies ref + drag state; no transform to avoid breaking sticky) ──
function SortableRow({
  id, children,
}: {
  id: string
  children: (opts: { isDragging: boolean; handleProps: React.HTMLAttributes<HTMLElement> }) => React.ReactNode
}) {
...
export default function GanttView() {
  const {
    tasks, users, selectTask,
    ganttOrder, setGanttOrder,
  ...
  // Effective root task order: known IDs first, unknown (new) IDs appended
  const allRootTasks = Object.values(tasks).filter((t) => t.parentId === null)
  const effectiveOrder = [
    ...ganttOrder.filter((id) => tasks[id]?.parentId === null),
    ...allRootTasks.filter((t) => !ganttOrder.includes(t.id)).map((t) => t.id),
  ]

  // Build visible rows in order, skipping archived (unless showArchived)
  const rows: Task[] = []
  ...
  effectiveOrder.map((id) => tasks[id]).filter(Boolean).forEach(visit)
  ...
  // Stats: leaf tasks by status (excluding archived unless showing)
  const counts = { todo: 0, in_progress: 0, review: 0, done: 0 }
  Object.values(tasks)
    .filter((t) => t.childIds.length === 0 && (showArchived || !archivedIds.includes(t.id)))
  ...
  const handleDragEnd = ({ active, over }: DragEndEvent) => {
    ...
    if (oldIndex !== -1 && newIndex !== -1)
      setGanttOrder(arrayMove(effectiveOrder, oldIndex, newIndex))
  }

  const renderRow = (task: Task, handleProps?: React.HTMLAttributes<HTMLElement>) => {
    ...
    return (
      ...
      >
        ...
        >
          {/* Phase accent stripe / drag handle */}
          {isPhase ? (
            <div
              {...handleProps}
              style={{
          ...
          <div style={{
            ...
            {isParent ? (
              <span
                style={{ color: isPhase ? '#5A7A9A' : '#3A5070', display: 'flex', alignItems: 'center', cursor: 'pointer', flexShrink: 0 }}
            ...
            <span style={{
              ...
              color: isArchived ? '#3A5070' : isPhase ? '#C8D8EC' : isParent ? '#D0DCF0' : '#7A96B8',
          ...
          {/* 担当 */}
          <div style={{
            width: ASSIGN_W, flexShrink: 0,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            gap: 3, borderLeft: '1px solid #182840',
          }}>
            {!isParent && (
              <>
                {firstAssignee && (
                  <span title={firstAssignee.name} style={{
                ...
                {secondAssignee && (
                  <span title={secondAssignee.name} style={{
                    width: 22, height: 22, borderRadius: '50%',
                ...
                {!firstAssignee && <span style={{ fontSize: 10, color: '#2A3F55' }}>—</span>}
              </>
            )}
          </div>
        ...
        {/* Gantt bar cell */}
        <div style={{
          position: 'relative', width: totalWidth, flexShrink: 0,
          height: ROW_H, background: ganttBg,
        }}>
          {weekTicks.map(({ x }, i) => (
            <div key={i} style={{
              position: 'absolute', top: 0, bottom: 0, left: x,
              width: 1, background: '#182840', pointerEvents: 'none',
            }} />
          ))}
          {todayX >= 0 && todayX <= totalWidth && (
            <div style={{
              position: 'absolute', top: 0, bottom: 0, left: todayX,
              width: 1, background: '#F0A500', opacity: 0.4, pointerEvents: 'none',
            }} />
          )}
          {pos && (
            <div
            ...
            >
              {barLabel && (
                <span style={{
                  ...
                  textOverflow: 'ellipsis', lineHeight: 1, pointerEvents: 'none',
                }}>
                  {barLabel}
                </span>
              )}
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: '#0C1219' }}>
        ...
        <div style={{
          ...
          background: '#0C1219', borderBottom: '1px solid #182840',
          flexShrink: 0, gap: 16,
        }}>
          ...
          <button
            ...
            style={{
              ...
              border: showArchived ? '1px solid #F0A50055' : '1px solid #1F3245',
              background: showArchived ? '#F0A50015' : 'transparent',
              color: showArchived ? '#F0A500' : '#7A96B8',
        ...
        <div ref={scrollRef} style={{ flex: 1, overflow: 'auto' }}>
          <div style={{ minWidth: LEFT_W + totalWidth, position: 'relative' }}>
            ...
            <div style={{
              ...
              background: '#172130', borderBottom: '1px solid #1F3245', height: HEAD_H,
            }}>
              <div style={{
                width: LEFT_W, flexShrink: 0,
                position: 'sticky', left: 0, zIndex: 20,
                background: '#172130', borderRight: '1px solid #1F3245',
              ...
              <div style={{ position: 'relative', width: totalWidth, height: HEAD_H, flexShrink: 0 }}>
                {months.map((m, mi) => {
                  ...
                  return (
                    <div key={m.toISOString()} style={{
                      ...
                      width: daysInM * DAY_W, height: MONTH_H,
                      ...
                      <span style={{ fontSize: 12, fontWeight: isCurrent ? 700 : 500, color: isCurrent ? '#F0A500' : '#7A96B8', letterSpacing: '0.02em' }}>
                ...
                {weekTicks.map(({ x, date }, i) => {
                  ...
                  return (
                    <div key={i}>
                      <div style={{ position: 'absolute', left: x, top: MONTH_H, bottom: 0, width: 1, background: '#1A2A3C' }} />
                      <div style={{
                        position: 'absolute', left: x + 4, top: MONTH_H + 7,
                        fontSize: 9, fontWeight: isThisWeek ? 700 : 400,
                ...
                {todayX >= 0 && todayX <= totalWidth && (
                  <>
                    <div style={{ position: 'absolute', left: todayX, top: 0, bottom: 0, width: 2, background: '#F0A500', opacity: 0.9, borderRadius: 1, zIndex: 3 }} />
                    <div style={{
                      position: 'absolute', left: todayX, top: MONTH_H + 6,
                      transform: 'translateX(-50%)', background: '#F0A500', color: '#0C1219',
                      fontSize: 7.5, fontWeight: 800, padding: '1px 4px', borderRadius: 3,
                      letterSpacing: '0.04em', whiteSpace: 'nowrap', lineHeight: 1.5, zIndex: 4, pointerEvents: 'none',
                    }}>
                      今日
                    </div>
                  </>
                  ...
                  {arrows.map((a, i) => {
                    ...
                    return (
                      <path key={i}
                        ...
                        fill="none" stroke={a.critical ? '#E3423B' : '#3A5070'}
                  ...
                  if (isPhase) {
                    return (
                      <SortableRow key={task.id} id={task.id}>
                        {({ handleProps }) => renderRow(task, handleProps)}
                      </SortableRow>
                    )
                  }
      ...
      {/* Drag overlay: floating phase name chip */}
      <DragOverlay>
        {activeId && tasks[activeId] && (
          <div style={{
            background: '#1A2B40', border: '1px solid #2A4870',
            borderRadius: 6, padding: '7px 14px',
            color: '#C8D8EC', fontSize: 12, fontWeight: 600,
            boxShadow: '0 6px 20px rgba(0,0,0,0.6)',
            display: 'flex', alignItems: 'center', gap: 8,
            cursor: 'grabbing',
          }}>
            <GripVertical size={12} style={{ color: '#4A6A8A' }} />
            {tasks[activeId].title}
          </div>
        )}
      </DragOverlay>
    </DndContext>
